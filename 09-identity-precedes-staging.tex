...
cleaner claim; removing it belongs to the deposit step (Code
provenance).
...
(\texttt{github.com/alvaroNCubo/puppeteer}) at commit \texttt{af3b9b0},
...
identifiers then. One cleanup belongs to that same step: the domain's
assembly carries an \texttt{InternalsVisibleTo} grant to the console
host which no host now exercises (Lab E reports it), and it is to be
removed when these commits are ported, so that the fence rests on the
test suite's grant alone. Until then the anchors are reproducible from
the accompanying archive, \texttt{paper09-data.zip}, whose contents are
