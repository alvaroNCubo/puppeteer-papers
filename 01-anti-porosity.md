...
**A criterion of demarcation.** The structure-vs-operation encoding decision is in principle observable wherever a representation is shaped for a universe of cases rather than the specific instance: function signatures with optional parameters, Protocol Buffer schemas with many `optional` fields, HTML forms serving heterogeneous use groups, generic-type parameters never specialized to specific types. The present paper draws a narrower line. The three layers enumerated in §2 — domain (the in-memory type hierarchy that an actor or aggregate exposes), endpoint (the wire contract that mediates external invocations), and persistence (the durable substrate of the source of truth) — together constitute the *complete representational surface of a runtime system that owns state across operations*. Other representational forms (transient DTOs the system traverses but does not retain, presentation-layer markup, type-system signatures the compiler discards after type-checking) are surfaces the system passes through but does not durably own. The frame may apply to them with appropriate adjustment; the present paper does not claim it across them. The demarcation that scopes the frame is therefore: *representational layers of a long-running stateful system that persist their shape into runtime*. The three conditions below address those layers; their adequacy outside this scope is open.
