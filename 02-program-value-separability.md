...
**Note on a methodological subtlety encountered during the Grzybek Lab 4 replication.** An earlier iteration of the bench passed the payer identifier as a `Guid` parameter and produced zero Action entries despite the test framework reporting success. The cause was localised to `Parameters.CanonicalTypeName` in the runtime, which accepts a fixed set of primitive types and currently rejects `Guid` with an exception that surfaces *after* the script has executed — a path the runtime's catch block treats as recoverable and silently records. The workaround is small (pass the GUID as a string and parse inside the facade) and is documented in `data/lab04-grzybek/headline.md`; both Lab 4 and Lab 5 on Grzybek run under that workaround. The diagnosis is captured here for the runtime maintainers; the structural measurements reported in §5.4 and §5.5 are unaffected by it.
