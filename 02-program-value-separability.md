...
The magnitude of the ratio is a function of how the host's domain API surfaces its data, not of the compaction mechanism. eShop's `AddOrderItem` carries the full product specification per item — pid, name, price, discount, picUrl, units — so per-iteration argument vectors are dense. Hosts whose verbs identify catalog entries by short stable references produce argument vectors an order of magnitude smaller, and the density ratio scales accordingly into the tens. Either way the structural property holds: arguments scale with invocations; the script body does not.
...
**Lab 4 replication on the second codebase is currently parked** (`data/lab04-grzybek/headline.md` documents the anomaly): the binary journal does not accumulate action entries when the bootstrap-then-measurement pattern is executed against the Grzybek facade, with no exception thrown by the runtime. Three hypotheses for the cause are captured in that headline; the eShop side of §5.4 carries the journal-density claim until the anomaly is resolved. The verb-richness replication of §5.5 on Grzybek is unaffected — its measurements rely on the static walker and the runtime dispatch counter, both of which are independent of the journal write path.
