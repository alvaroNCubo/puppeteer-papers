...
The mechanism is straightforward to follow in the source. `PrepareCommandProgram` (`ActorHandler.cs:923-973`) orchestrates the first encounter: it rents a parser from a pool, parses the script, and decides — by parameter presence — whether to enter the parametric path. On the parametric path, the program's `PrgExpression` method (`Program.cs:180-244`) walks the AST: for each Statement, it calls the Statement's `ExecuteExpression`, which returns an `Expression` node bound to the runtime's parameter and output expressions. These nodes accumulate into a `BlockExpression`, which `PrgExpression` wraps in `Expression.Lambda<Func<Parameters, Output, string>>` (`Program.cs:242`). The compilation itself is one line: `_ejecutable = programExpression.Compile()` (`Program.cs:162`). The compiled delegate lives in the program's `_ejecutable` field; subsequent invocations skip the compile step and call the delegate directly with the values supplied at the call site.
...
The mechanism is brief in the source. When an actor is constructed, its assembly's public types are walked once by reflection and cached in a per-assembly dictionary: `DomainLibraries.GetOrLoad(Assembly)` (`DomainLibraries.cs:77-89`), invoked from `ActorHandler` (`ActorHandler.cs:60`). The cache survives the lifetime of the process; the same assembly loaded by a second actor reuses the same domain dictionary. Against that fixed surface, new DSL scripts arrive through the actor's fluent interface — `ActorV2.Using(scriptForChk, scriptForCmd)` (`ActorV2.cs:32`) — at any time the actor is running. There is no AppDomain reload, no `AssemblyLoadContext` unload, no file-watcher over the assembly: the surface area is fixed at first load and dynamics live entirely above it.
...
| AST traversal | `Program.cs:180-244` | `PrgExpression` walks `Statement`s, accumulates `Expression` nodes into a `BlockExpression` |
| Lambda composition | `Program.cs:242` | `Expression.Lambda<Func<Parameters, Output, string>>` wraps the `BlockExpression` |
| Compilation step | `Program.cs:162` | `_ejecutable = programExpression.Compile()` |
...
| Commands caching rule | `ActorHandler.cs:943` | Caches only when `parametros.HasUserParameter()` is true |
| Queries caching rule | `ActorHandler.cs:1352` | Caches when `PARAMETROS_VACIOS` *or* `HasUserParameter()` is true (asymmetry vs commands) |

### §3.3 — Hot-loaded DSL programs

| Reference | Location | What it shows |
|---|---|---|
| Domain library loading | `DomainLibraries.cs:77-89` | `GetOrLoad(Assembly)` reflects public types and caches them once per assembly |
| Library binding to actor | `ActorHandler.cs:60` | `libraries = DomainLibraries.GetOrLoad(actor.GetType().Assembly)` |
