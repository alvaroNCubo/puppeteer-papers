...
**`37ad9cf`** (the paper's Code Provenance), with the harness in
`labs/lab04-tell`.

Paper 4 is qualitative: the lab reproduces the **journals** the paper exhibits,
not timings. `run-37ad9cf.txt` is the captured stdout — the three cross-actor
...
(`tell PurchaseConfirmed with ... to RewardEngine('rewards-1') once @order`).

## What it shows (Paper 4 §8.2 + §8.3 + §8.5)

- **Saga** — the joint history lives only in the coordinator's journal
  (3 entries); the Seller (3 — its purchase as define+invoke) and RewardEngine (6 — two campaigns + the reward) hold half-stories.
...
- **Tell** — the Seller's own journal holds the joint history: the purchase, the
  ...
  `tell ack` (6 entries), and the round-trip closes inside the program.
