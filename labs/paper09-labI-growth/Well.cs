...
internal sealed class Well
{
    ...
    /// <summary>
    /// The official score — the well's own word on what the game has been worth
    /// so far, accumulated by the <see cref="Scoring"/> tariff as rows collapse.
    /// Unlike <see cref="ClearedLines"/> it is not recoverable from a board: it
    /// depends on <em>how</em> the rows fell (four at once outscore four one at a
    /// time), so it is history, and the well is where that history is kept.
    /// </summary>
    internal int Score { get; private set; }

    /// <summary>
    /// The difficulty level the game has reached — <em>derived</em>, not stored,
    /// by the <see cref="Difficulty"/> rule over <see cref="ClearedLines"/>. The
    /// well says what level it is at and nothing about what that should cost: how
    /// fast gravity falls at level 5 is a staging's business, not the domain's.
    /// </summary>
    ...
    /// what collapsed, score it, and clear the active slot. It does <em>not</em>
    /// spawn the next piece; the caller does that with <see cref="Spawn"/> once it
    /// sees <see cref="IsAwaitingPiece"/>.
    /// </summary>
    private void Land()
    {
        (Pile, var collapsed) = Pile.Integrate(Active!);
        ClearedLines += collapsed.Count;
        Score += Scoring.Award(collapsed.Count);
        Active = null;
