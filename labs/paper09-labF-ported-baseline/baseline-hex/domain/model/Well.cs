...
internal sealed class Well
{
    ...
    {
        RequireAdmissibleSize(width, height);
    ...
    /// <summary>
    /// Reopens a well at a position it previously held — a pile of landed cells, a
    /// cleared-line count, and whichever piece was falling. The invariants are
    /// re-checked, so a state that could not have arisen in play is rejected here
    /// rather than silently resumed.
    /// <para>
    /// Added for staging 4. A client that runs one operation per process cannot
    /// keep the well in memory between operations, so the arrangement needs a
    /// state port — and a state port needs a way back in. This constructor is that
    /// way, and it is the point at which persistence reached the model.
    /// </para>
    /// </summary>
    internal Well(int width, int height, ImmutableHashSet<Position> pileCells, int clearedLines, Piece? active)
    {
        RequireAdmissibleSize(width, height);
        ...
        if (clearedLines < 0)
        {
            throw new System.ArgumentOutOfRangeException(
                nameof(clearedLines), clearedLines, "A well cannot have cleared a negative number of rows.");
        }

        Frame = new Frame(width, height);
        Pile = Pile.Of(width, pileCells);
    ...
    private static void RequireAdmissibleSize(int width, int height)
    {
        if (width < 4)
        {
            throw new System.ArgumentOutOfRangeException(
