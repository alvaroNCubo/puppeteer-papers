# hex-pile-scan.ps1 - the automated player's computed view, for the hexagonal baseline.
#
# The counterpart of Tetris/tools/pile-scan.ps1, pointed at the hexagonal
# staging's frame file. It reads the frame the board output port wrote and lifts
# the 200-cell bitmap into the SYMBOLIC view a commander reasons in: the skyline
# vector, its difference profile, the zeros (fill priority), wells, a few
# aggregate metrics, and the active piece.
#
# The point it makes for the comparison is the same one Paper 9's Lab D makes: the
# projection belongs to the party that reads it. The hexagon emitted a board; this
# view is not one the hexagon offers, and nothing in the domain knows it exists.
# That much ports-and-adapters gets right too, and this file is the evidence.
#
# Key trick (unchanged): a column's pile-height is the run of filled cells
# contiguous FROM THE FLOOR up. That (a) equals the true surface in no-hole play
# and (b) excludes a falling piece, which floats above a gap. Anything filled
# ABOVE the first gap is reported as "floating" (the active piece, or an overhang).
#
...
#
# Usage:  hex-pile-scan.ps1 -Session hexplay1
#         hex-pile-scan.ps1 -FramePath C:\...\some.frame
...
if (-not $FramePath) {
  ...
  # The frame file sits in .sessions beside baseline-hex\TetrisHex.sln.
  $slnDir = Split-Path -Parent $PSScriptRoot
  $FramePath = Join-Path $slnDir ".sessions\$Session.frame"
}
...
# Render the computed view.
$signed = ($diffs | ForEach-Object { if ($_ -ge 0) { "+$_" } else { "$_" } }) -join ' '
"== HEX MIRILLA  frame=$([System.IO.Path]::GetFileName($FramePath)) =="
