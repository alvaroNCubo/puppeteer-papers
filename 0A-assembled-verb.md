...
Four numbers and a consequence. Asked of one record: how many of the constituting statements does it contain, and does the record alone replay into the whole? An event-sourced coordinator answers **0 / no**, durable execution **6 / no**, a trace **6 / no**, the constituted verb **6 / yes**. The verb is defined once and exercised many times — **1 definition, 2 invocations** — with each exercise's command/query modality attributed at assembly rather than inherited. The corpus's **72** named artifacts include **zero** naming a trajectory that spans subjects. The record's price is **~210x** the same statements as direct calls, and ~112 bytes per exercise. The consequence the series takes forward: once the composition has a place of its own, continuity no longer has to be authored into its constituents — and the constituents need not know the future wholes in which they will participate.
