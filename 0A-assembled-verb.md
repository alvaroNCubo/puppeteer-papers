...
date: 2026-08-27
version: 0.1-draft
status: v0.1-draft — complete draft; deposited on Zenodo; not yet peer-reviewed. Every file:line anchor is resolved at the deposited engine commit 3160e39, where the twenty-one labs of Appendix A run. Every count resolves against those labs or against the dissection bundles of paper0A-assets.
keywords:
  - capability
  - composition
  - repertoire
...
abstract: >
  ...
  test suites still exercise their domains once store, bus and test double are
  removed.
  ...
  three capabilities, and after the run there are the same three. The paper
  ...
  wider population is claimed. Sixteen neighbour families are conceded by
  name, and what none of them was found to hold together is the conjunction:
  a subject-owned record of a definition and its exercises, constituted from
  repertoires the subject does not own, with modality — including what counts
  as the subject's history — attributed at the plane of assembly. A prototype of a few hundred lines realizes the
...
`A(); B(); C();` consumes three capabilities and leaves three. This paper measures an arrangement where assembly *produces* one: the composition becomes a verb of a subject — one definition in the subject's record, exercised many times by reference — and the paper's instrument is a question askable of any record, which separates that from a coordinator's record, a durable-execution history and a trace. Where such a unit is missing, the continuity still has to go somewhere, and shipped software authors it into the pieces: §6 measures those compensations, and a census of 72 named artifacts finds no name for a trajectory spanning subjects. So the consequence the series takes forward is not that composition became first-class, but where continuity may live: once the composition has a place of its own, it no longer has to be authored into its constituents — and the constituents need not know the future wholes in which they will participate.
...
**(a) separates only the first shape.** The event-sourced coordinator's own journal holds its transitions and no operation: which step it reached, what it awaits — measured on Orleans, six progress marks and zero operations, the operations living in other subjects' acts. The durable-execution record, by contrast, holds all six invocations — names, inputs, results. Content cannot tell it from the description row or from constitution.

**(b) separates the rest, and it separates them differently.** The trace answers *no* vacuously: it supplies no replay operation, so no replay of it exists after which the outcome could obtain. The durable-execution record has a replay, and in the model's own operation the orchestration recovers and completes — the row's *no* does not deny what the model guarantees. What the row scores is (b)'s input clause: that replay re-executes **the orchestrator's code** against the recorded history — a definition living outside the record — so its outcome is determined by the pair, not by the record alone. Measured: the orchestrator body ran in seven episodes, six of them replays, while every activity executed exactly once; the record alone re-performed nothing, and that is the model's own determinism contract. The constitution row's entry is read back, and reading it back performs the six statements again — nothing outside the record but the engine and the repertoires.

The input clause is not a footnote the strongest row forced; it is the half of the criterion that decides that row: **where the definition lives.** In durable execution the composition is compiled code, and the history holds exercises and outcomes that only that code can re-walk (§7.2). In the constitution row the definition is itself an entry in the record, which is why — with the repertoires held fixed, as (b) fixes them — the record alone suffices to re-perform. The difference from the strongest coordination is in what stays ambient: there, the composition and the operations; here, the operations only.
...
**The cost measured is the record's; no advantage is claimed.** Three prices are measured against the direct baseline the criterion already uses (Appendix A: `WhatTheRecordCostsLab.cs`, the run published as `data/paper0A-assembled-verb/price-release.log`), every timed quantity over ten runs and reported as its median with the min–max spread, on a declared environment: Release build of engine and harness, .NET 9 on Windows 11, one 32-thread desktop processor, workstation GC, wall clock, the corpus assemblies prebuilt binaries in both arms — and the direct baseline performs the same three statements and allocates the same domain objects, so what it lacks is only the record. *Growth* (exact counts, not samples): the act carrying the definition and first exercise costs 554 bytes of journal; each additional exercise costs ~112 — reuse is visibly cheaper than restating, which is the capability claim's material side. *Replay:* reconstructing the subject from a record of 100 exercises takes 4.6 ms [4.4–5.6]; from 1,000, 15.0 ms [13.9–20.2]. *Throughput:* the plane performs ~2,490 exercises per second live [2,310–2,562], where the same statements as direct in-memory calls run at ~529,000 loops per second [476,000–542,000] — a **~210x** price, the ratio of the two medians, reported to two significant figures because that is what the instrument supports: repeated invocations of the lab, each with its own warm-up and ten samples per arm, move the multiplier by a few percent. Two sampling decisions are part of the measurement rather than incidental to it. A warm-up run is discarded in every arm, so that JIT compilation is not sampled as steady-state work. And a direct *sample* is ten loops of the same thousand, each over a fresh register, timed together and divided back: one loop of a thousand lasts about two milliseconds — the scheduler's noise floor rather than a measurement — while scaling a single loop to ten thousand would have changed the work itself, since the register would grow ten times larger. The comparison is asymmetric by construction and the asymmetry is the finding: the direct loop buys execution only; the plane's loop buys execution plus a definition exercised by reference, an attributed modality, and a record that reconstructs the subject. The decomposition the two medians support: replay re-performs the same thousand exercises in ~15 ms — about eight times the direct loop's median, against the plane's ~210 — which locates the dominant term outside the plane's interpretation, in the durable write — an inference from two measured quantities rather than a third measurement. What remains unmeasured: concurrency, scale beyond one machine, and every competing arrangement's own price — whether this arrangement is cheaper or faster *than any other* is unexamined.
...
**One value in the composed verb returns fresh on every replay, and no one at the plane has the authority to stabilize it.** An identity minted inside a repertoire's factory changes on every replay; the criterion's equivalence absorbs the freshness, and Appendix C proves that exact identity is unavailable under its host axiom while that equivalence is — so what stands is not a replay failure but an **authority boundary**, reported against the paper's own flagship entry: the assembler can neither supply the value nor stabilize it, where durable-execution runtimes stabilize theirs by design and a domain's author could by asking (§4.3). The calculus also states the condition under which the guarantee does hold — every reference to a minted value travelling between acts as a lookup under a key that crossed the plane — which is a characterization of when replay is safe, not advice about how to write verbs (Appendix C). What the arrangement supplies is otherwise detection rather than immunity: the corpus's own suites never meet this value — its 49 tests pass over it — and replay is the instrument that makes the undeclared inbound visible, against the actor premise, a standard the repertoire never adopted, which is why §4.3 charges neither party. The value is also the measured instance that forces §3's outcome equivalence to be declared before use (§3.1). The subject inherits a decision it cannot author around. Completeness, executability and testability are the repertoire's properties; a replay-stable minted identity is not among them, and no assembler can add it from outside — this corpus separates the four.

**A captured mint breaks replay equivalence outright, and this one the calculus found first.** Past the licensed freshness sits a sharper hazard: a minted identity read by the assembler and *captured* into a later act's recorded arguments. The recorded literal names a value the live run held and the replayed run never minted, so no renaming relates the two (Appendix C, Proposition 4's boundary). Measured, the composition's own invariant — the register's key is the identity of the payment kept under it — holds live and fails after replay; the same composition referring to the value by a supplied key holds through both, its mint just as fresh (Appendix A: `WhereReplayEquivalenceBreaksLab.cs`). This is the paper's one finding obtained by reasoning about the arrangement rather than by measuring what it was built to do: the derivation is committed before the lab that confirms it exists, and both commits are cited in Appendix A, so the order is checkable rather than asserted. Durable-execution practice prevents the case by design, supplying replay-stable identifiers from the runtime; this arrangement leaves it to what the assembler records, and the calculus states the sufficient condition under which the guarantee holds.
...
**The query modality is clean about the record, not about state.** A query leaves no entry, and that is what §2.4 measures. What follows for state is narrower than it may read: the engine prevents a query from persisting, and in an arrangement where durable state *is* reconstruction from the record, what a query touched does not survive rehydration. Repertoire state held outside that record — statics, caches, external stores — is reached by a query like any other act, and returns from nowhere. Appendix C carries the same boundary as its second idealization.
...
And every item just listed survives the flatter description §4.1 admits, for a reader who declines this paper's vocabulary: the criterion, the ambient hierarchy, the attribution of historicity and the census are findings about records and engines, not about the words for them. The declarations of §8 fence that field; they do not empty it.
...
**Run.** `./fetch-corpus.ps1` once, then `dotnet test`. Twenty-one labs, all green. The suite compiles against the engine's public surface only — no signing and no internals grant.
...
| `RetrievalIsALookupLab` (+ `SubscriptionPaymentBridge`) | §4.3 | the licensed mint, and nothing about capture: the assembler's key stable across replay, the domain-minted identity fresh, retrieval a lookup in the subject's own state; the minting site marked `R_99` |
...
| `EveryNameStaysWithinOneSubjectsVocabularyLab` | §6 | the census under two coding rules: population **27 + 16 + 29 = 72** reproduced mechanically; rule one (≥2 aggregates in one name) codes **0** T; rule two hands its blind spot — zero-aggregate names, process words — to manual coding: **3** residuals, each published with its reason, all S; agreement **72/72** (§6 on what that agreement is not) |
...
| `WhereReplayEquivalenceBreaksLab` | §4.3, §8, App. C | the calculus's prediction, put to the engine: a minted identity captured into a later act's recorded arguments breaks the composition's invariant on replay (true→**false**), while the same composition referring to it by a supplied key holds (true→true) — the mint fresh in both arms. Order: derivation `0001735` (15:08), lab `0813fec` (17:06), same day |
| `WhatTheRecordCostsLab` | §8 | the record's price, Release, n=10 after a discarded warm-up, median [min..max]: **554** bytes for definition + first exercise, **~112** per additional (exact); replay **4.6 ms** at 100 and **15.0 ms** at 1,000 exercises; **~2,490**/s through the plane vs **~529,000**/s direct — **~210x**; published as `data/paper0A-assembled-verb/price-release.log` |
...
- *General web search.* Discovery strings, verbatim, run on 15 and 17 August 2026 — the claim being scoped to what was examined, not to when the examining stopped: `composite command pattern macro command first-class reusable composition object-oriented design`; `Dietz DEMO enterprise ontology actor role competence authority responsibility transaction pattern production act`; `event sourcing "command definition" recorded journal replay "invocation" reuse parametrized action definition once invoked many`. Verification strings — used to confirm candidates' bibliographic facts, not to discover — covered the 1972 macro-operators paper, the options framework, Talents, and durable functions. One further string was seeded with its intended answer, is recorded here as biased by construction, and its subject was excluded from the paper on provenance grounds.
...
§2.4's four questions — what is computed, what the subject can do, what it does, what counts as its history — are distinctions, and §2 and §3 measure them on running arrangements. A distinction, however, is *stated over an object*, and the smallest object that can carry these four is worth writing down. This appendix gives it: a core calculus, in the structural style of Plotkin (2004), just large enough that assembly, exercise, modality attribution and replay have operational rules — and small enough that each load-bearing separation is a short proposition, with what is merely bookkeeping labelled an observation. The labs of Appendix A are its conformance checks, one per claim: `TheCompositionBecomesACapabilityLab` for Observations 1 and 2, `WhoDecidesWhatCountsAsHistoryLab` for Proposition 3, `RetrievalIsALookupLab` with `WhereReplayEquivalenceBreaksLab` for Proposition 4 and its boundary, and the two coordination arms with `TheCriterionForConstitutionLab` for Proposition 5.
...
Two scope decisions are made at the grammar and declared as such. Bodies are first-order: no statement invokes a verb, and composition of verbs out of verbs is out of scope here. And the calculus carries no speech of the plane's own — §5.1's `print` and `tell` are outside the object, which covers the composition of operations and not the subject's vocalization; the four questions need no more, and §5.1's claim exceeds this appendix on purpose.

**Configurations, and what is given.** `⟨D, σ, J⟩` — definitions `D : Verb ⇀ (x*, b)`; domain state `σ`; the journal `J`, a sequence of entries `def(V, x*, b)` and `act(V, v*)`. One constant comes with the calculus: an initial state `σ₀`. A configuration is **reachable** when the rules below reach it from `⟨∅, σ₀, ε⟩`; every claim quantifies over these.

**The host as a parameter — axiom (HOST).** The repertoires enter the calculus as an interpretation governed by one axiom, named once and cited by name:

    (HOST)  ⟦f⟧ relates (σ, v*) to (σ′, v, N), totally on (σ, v*), where
            N is the finite set of fresh names the application mints:
            N ∩ names(σ, v*) = ∅;  |N| is determined by (σ, v*) and is
            invariant under bijective renaming of the names in (σ, v*);
            (σ′, v) is determined by (σ, v*) up to bijection on N; and
            the relation is equivariant — renaming the names of (σ, v*)
            renames the outcome accordingly.

Freshness is the *only* mechanism of the calculus for new names — there is no name supply in the configuration, so nothing re-determinizes the mint. Two applications of the same `f` to the same arguments agree except in *which* fresh names they drew: which is what the prose of §4.3 says, what the measured factory does, and what makes Proposition 4's bijection necessary rather than decorative. (HOST) is the closure condition §4.3 speaks of, written as an axiom: whatever an execution depends on either crossed the plane (it is in `v*`) or is fresh (it is in `N`). It is a **closure**, and it is worth saying plainly what it closes out: external I/O, randomness beyond the minting the axiom licenses, foreign mutable state reached behind the plane, callbacks, and clock readings that escape it — §4.3's undeclared inbounds. One further axiom gives the calculus something to distinguish, and Proposition 5 cites it:

    (NONTRIV)  there exist f, g in Op and (σ, v*) whose applications differ in σ′.

**Execution.** A statement executes under a local environment `ρ : Var ⇀ Val` — the values flowing between statements, the third constituent §2.2 names — and emits its operation's result as output:

    ⟨σ, ρ⟩ ⊢ s ⇓ ⟨σ′, ρ′⟩ ! v

For `x := f(e*)`: the arguments `e*` are read under `ρ` (a variable `ρ` does not bind is stuck), `⟦f⟧` is applied, minting per (HOST), and `ρ′ = ρ[x ↦ v]`; for bare `f(e*)`, `ρ′ = ρ`. A body folds its statements left to right, `⟨σ, ρ⟩ ⊢ b ⇓ ⟨σ′, ρ′⟩ ! v`, emitting the last statement's output. The fold always terminates: bodies are finite, invoke no verbs, and `⟦f⟧` is total.

**Rules.**

    ops(b) ⊆ Op      V ∉ dom(D)
    ─────────────────────────────────────────────────────────────  (DEFINE)
    ⟨D, σ, J⟩  ─define V(x*)=b→  ⟨D[V↦(x*,b)], σ, J·def(V,x*,b)⟩

    D(V) = (x*, b)    |v*| = |x*|    ⟨σ, [x*↦v*]⟩ ⊢ b ⇓ ⟨σ′, ρ′⟩ ! v
    ─────────────────────────────────────────────────────────────  (PERFORM-CMD)
    ⟨D, σ, J⟩  ─perform V(v*):cmd→  ⟨D, σ′, J·act(V,v*)⟩ ! v

    D(V) = (x*, b)    |v*| = |x*|    ⟨σ, [x*↦v*]⟩ ⊢ b ⇓ ⟨σ′, ρ′⟩ ! v
    ─────────────────────────────────────────────────────────────  (PERFORM-QRY)
    ⟨D, σ, J⟩  ─perform V(v*):qry→  ⟨D, σ, J⟩ ! v

Two readings of PERFORM-QRY are deliberate. The execution happens — the premise is the same judgment as the command's, and the output `v` leaves, which is what keeps a query performance observable at all — and the configuration keeps nothing: not the effect, not the trace. What a query computes is visible at the boundary as its output and durable nowhere. In the engine measured this is literal, because state *is* reconstruction from `J` (§2.4): what a query changed does not exist after rehydration.

**Four idealizations, declared.** *(1)* The define/perform split is primitive; in the engine the two arrive fused in one act and the handler splits them at the reuse seam, and §2.3 measures that it does. *(2)* PERFORM-QRY is a rollback rule: the premise executes the body and the conclusion restores `σ` whole, silently undoing whatever a state-changing operation did. That is a substantive modeling decision, and it is **laxer than the engine it models**, which prevents rather than undoes: a query runs under a read lock with the symbol table in read-only mode, and its parser rejects the statements that would persist (§2.4). The calculus admits a query whose body would change state and unwinds it; the engine refuses to run one. The rules' conclusions agree — the configuration retains nothing either way — and every claim below depends on that conclusion rather than on the path to it, so a proposition proved under the laxer rule holds a fortiori under the stricter engine. What neither reaches is repertoire state held outside `σ` altogether (statics, caches, external stores), which rolls back nowhere. *(3)* `⟦f⟧` is a relation over one store: no aliasing across repertoires, no shared mutable state outside `σ`, no reentrancy. The shape assumes by construction the tractability of exactly the situation §5.2 exhibits as hard — two repertoires whose only meeting point is the assembler. (HOST) bounds what `⟦f⟧` may *depend on*; this shape bounds what it may *be*. *(4)* `⟦f⟧` is total: no operation fails, so the calculus expresses no failed exercise — misfire and its kin are outside the object.

**Replay.** `replay(J)` is **a** configuration reached from `⟨∅, σ₀, ε⟩` by taking each `def` entry as its DEFINE step and each `act` entry as its PERFORM-CMD step, in `J`'s order — *a* configuration, not *the*: under (HOST) each run of the replay draws its own fresh names, and the propositions quantify accordingly. For the coordination variant of Proposition 5, `replay_P(J)` starts instead from `⟨D_P, σ₀, ε⟩`, with `D_P` read off the ambient program: the variant's journal holds no `def` entries, so a replay started from `∅` is stuck at its first `act` — which is itself the point, stated operationally: the record alone does not carry what its replay needs.

The five numbered claims are not of one kind, and the difference is declared rather than left to be discovered. Observations 1 and 2 are **bookkeeping**: they record what the definitions make true — Observation 2 follows from the side condition `V ∉ dom(D)`, a fiat of the calculus — and their value is exactness, not discovery. Propositions 3, 4 and 5 carry the **content**: each could have been false of rules shaped otherwise, and each has an engine measurement that could have disagreed.

**Observation 1 (defining computes nothing).** DEFINE leaves `σ` fixed; hence there are reachable configurations equal in `σ` and different in `D`. *Why.* Immediate from the rule's conclusion; the witness pair is any configuration and its DEFINE-successor. On the engine: §2.2 exhibits the definition as an entry — content, not effect.

**Observation 2 (a capability is not its exercises).** After `define V(x*) = b` and n command performances with arguments `v*₁ … v*ₙ`, `J` holds exactly one `def` entry for `V` and the n entries `act(V, v*ᵢ)`, none containing `b` and all referencing the same `V`. *Why.* DEFINE's side condition `V ∉ dom(D)` forbids a second definition of `V`, and PERFORM-CMD appends entries that carry a name and arguments only. The definition/exercise separation is therefore *data in J*, not an interpretation of it. The idealization at work is (1) of the declared four: in the engine, assembly and exercise arrive fused in a single act, and the handler splits them at the reuse seam — a known body is an invocation, a new one a definition (§2.3). The calculus takes the split as primitive; §2.3 measures that the engine realizes it.

**Proposition 3 (what it does is not what counts as its history).** Fix `D`, `σ`, `V`, `v*` with `D(V)` defined and `|v*|` matching its parameters. The command and query performances execute under the same premise — one judgment, the same body from the same state, the same output emitted — and differ in journal membership, which is therefore a function of `m` alone. And `m` is a component of the *performance*, supplied at the exercise rule: it is not computed from the body, the state, or the outcome. The calculus establishes the **location** of the choice, not its author — there is no attribution judgment here, and none is needed for the separation. *Proof.* The rules share their premise and differ only in conclusion; the emitted output is what keeps the query half of the witness observable, since the query's configuration is unchanged. ∎ Who occupies that position in the measured arrangement is §2.4's question, and its answer there is the assembler. On the engine: §2.4 — the journal head unmoved under the query, advanced under the command, the domain computing the same value both times.

**Definition (≅, determination, and what it is for).** Two configurations are **equivalent**, `C ≅ C′`, when some bijection π relates them componentwise, mapping the names minted during `C′`'s trace to those minted during `C`'s and fixing every other name — the two traces are distinct, and π is exactly the correspondence between what each of them minted. A pair `(J, ⟦·⟧)` **determines** replay's outcome up to ≅ when every configuration any run of `replay(J)` reaches falls in one ≅-class. And ≅ is what §3.1's *outcome exists again* means, made precise: **the whole's outcome exists again exactly when the replayed configuration is ≅ to the original** — which is the same clause §3.1 states in its own words as identity up to fresh values, and the reason it must be stated before the criterion is used. Propositions 4 and 5 use exactly these notions. (The local environment of the execution judgment is written `ρ`; the bijection here and in what follows is always `π`.)

**Proposition 4 (the record constitutes, up to fresh names — and no closer).** Assume (HOST); let `⟨D, σ, J⟩` be reachable, and let the trace be **journal-closed**: no entry of `J` — neither a `def` body nor an `act`'s arguments — contains a name minted during that trace. Names are values in this grammar, so a body can carry one and the hypothesis has to range over the whole journal, not only its arguments; what it says is that everything the record holds crossed the plane from outside, which is exactly what makes π fix `J`. Then every run of `replay(J)` reproduces `D` exactly — definitions mint nothing, so that much is a strict equality — reproduces `J` (by journal-closedness no entry contains a trace-minted name, so π fixes every one of them: `π(J) = J`, hence `J_r = J`) — and reaches a state `σ_r` with a bijection π on minted names such that `π(σ_r) = σ`. The three conclusions together are `replay(J) ≅ ⟨D, σ, J⟩`, which is what Proposition 5 cites. The renaming cannot be dropped: **exact identity is not available for every interpretation satisfying (HOST)**, and equivalence up to bijective renaming is. The witness is immediate — an interpretation that draws a different fresh name on each application satisfies the axiom and defeats name equality across runs — so §3.1's clause is proved necessary rather than conceded. No maximality beyond that is claimed, and none is needed. *Proof sketch.* Induction over the define/command sub-trace with hypothesis `π_k(σ_r^k) = σ^k`; query steps change nothing in the configuration, so they drop out. At the next command step both sides execute the same journaled `v*`, which by journal-closedness contains no trace-minted name, so `π_k` fixes it; (HOST)'s equivariance applied to `π_k` transports the replay's application at `(σ_r^k, v*)` to an application at `(σ^k, v*)` — the live one — whose outcomes therefore correspond under an extension `π_{k+1} ⊇ π_k` over the step's fresh names, equal in number by (HOST)'s count invariance. The extensions compose because fresh names are new on both sides. ∎ On the engine: the constitution row of §3.2, whose *yes* is stated under exactly this equivalence (§3.1), and §4.3's measured identity — fresh on every replay while the supplied key holds — which is π made visible.

**Where the hypothesis fails.** Let a trace-minted name be *captured* — read by the assembler and passed into a later act's journaled arguments. The live application's argument then aliases a name of the live state; the replayed application's argument is the recorded literal, which aliases nothing in the replayed state, since replay minted fresh; and no bijection can both align the states and fix the literal. Replay equivalence breaks — not at the mint, but at the **capture** of the mint.

The converse is the calculus's usable half: if every reference to a minted value travels between acts as a lookup under a key that crossed the plane, rather than as the value itself, the trace is journal-closed and Proposition 4 applies. That is a sufficient condition — a characterization of when the guarantee holds, not advice about how to write verbs.

Both halves were then measured, in a lab written after this derivation (§8 for the finding and its standing; Appendix A for the arms, the numbers and the commit order). One clause of that measurement belongs here rather than there: what the lab observes is a composition's invariant holding live and failing after replay, and that this *is* a violation of ≅ is the calculus's step — a configuration whose invariant fails cannot be related by any renaming to one whose invariant holds, since π maps values to values and cannot make an equality false.

**What the hypothesis costs the measured arrangements.** Nothing, in all three: §2.2's flagship is a single act whose arguments are all supplied, so its trace is journal-closed outright; §5.2's scenario is journaled nowhere, exercising the two repertoires directly to exhibit what neither can say; and §5.1's co-binding travels inside one body, through the local environment. The hypothesis bites only where the measurement puts it — a value minted inside an operation, read out, and recorded as a later act's argument — and what would violate (HOST) itself is different again: uncaptured I/O, escaped clocks, foreign mutable state.

The proposition's scope has to be stated as exactly as its proof: **Proposition 4 characterizes *replayable* constitution under (HOST); it does not characterize constitution itself.** And it is doubly relative: to (HOST), and to the interpretation — replay reads `⟦f⟧` from outside the record, and Proposition 5 makes that second relativization explicit. Where (HOST) fails, what fails is replay equivalence — the (b)-half of §3's innermost level — and nothing above it: DEFINE still extended `D`, the definition and its exercises still stand in `J`, and Observations 1 and 2 and Proposition 3 hold without the axiom, since none of their arguments touches `⟦f⟧`. A verb whose repertoire reaches behind the plane in the ways (HOST) closes out is still defined, still exercised, still owned; its record has lost a guarantee, not the verb its existence.

**Proposition 5 (what the criterion separates is how much stays ambient).** Under Proposition 4's hypotheses, `(J, ⟦·⟧)` determines constitution's replay up to ≅; coordination's replay additionally depends on the ambient program `P`.

*Proof.* The positive half is Proposition 4, which concludes `replay(J) ≅ ⟨D, σ, J⟩` at the level of whole configurations — the level ≅ is defined at — and inherits its hypotheses, journal-closedness included. The negative half needs neither (HOST)'s freshness clauses nor anything about a particular engine: by (NONTRIV) there are operations whose outcomes differ at some state, reachable in the variant since the two calculi share their execution rules and differ only in where a definition lives; so ambient bodies `b ≠ b′` differing in such an operation make the same `J` replay to different states under `P[V↦b]` and `P[V↦b′]`, the interpretation held fixed. ∎

*Interpretation.* Neither calculus lets the record settle its replay absolutely — re-performing reads `⟦f⟧`, code outside `J`, in both — which is why §3.1's (b) holds the interpretation and `σ₀` fixed. What separates them is the remainder beyond `(J, ⟦·⟧)`: what moves into the record is the *composition*, never the operations, a difference in how much stays ambient rather than a categorical "record alone", and §8 carries that remainder as a limit. Because the negative half rests only on a `P` free to vary, the separation survives any further narrowing of the positive half. On the engine: the durable-execution row of §3.2, where the history holds the exercises and replay re-walks compiled code, so the same history under changed code means something else; and the constitution row, where with the repertoires held fixed, changing nothing but the record is enough. §3.1's (b) asks this proposition's question of a record, and its two halves are answered here and at Proposition 4 with the measurement of §3.2.

**The four questions, and the one arrow among them.** Over a reachable configuration, three of §2.4's four lines are projections: *what the subject can do* is `dom(D)`; *what counts as its history* is `J`; *what is computed* is `σ`. The fourth — *what the subject does* — is deliberately not in the configuration: performances are the trace's property, and Proposition 3 witnesses their separation at the rules themselves, one premise under two conclusions. What is claimed is exactly the separations §2.4 needs, each with its witness: equal `σ` with different `D` (Observation 1), so what is computed does not fix what the subject can do; equal `J` with different executions (Proposition 3's query half), so the history does not reveal everything the subject did; equal `D` with different `J` (runs of different exercise counts), so the repertoire does not fix the history. And one positive dependency, which the paper uses: under Proposition 4's hypotheses `J` reconstructs `D` exactly and `σ` up to ≅. That arrow is the finding rather than the table's defect — recorded constitution *is* the arrangement in which the history suffices for the rest — and no wider independence is asserted here.

**What is not claimed.** No metatheory beyond these claims is developed. In particular, no equivalence between a programming model and its record-replay implementation is proven — the durable-execution neighbour proves exactly such a theorem for its model (Burckhardt et al., 2021), and this appendix's object is smaller and its aim narrower: to carry the distinctions. The calculus is a characterization of the measured arrangement. Conformance of the engine to the rules is checked by the labs, one per claim, and checked is all it is.
