...
**Proposition 5 (what the criterion separates is how much stays ambient).** Neither calculus lets the record settle its replay absolutely: re-performing reads `⟦f⟧`, which is code outside `J`, so a changed operation implementation replays the same record to a different state in *either* calculus — the interpretation is a parameter everywhere, which is why §3.1's (b) holds it fixed, along with the given `σ₀`. What separates the calculi is determination beyond `(J, ⟦·⟧)`, in the defined sense. In the constitution calculus, `(J, ⟦·⟧)` **determines** replay's outcome up to ≅ — Proposition 4 concludes `replay(J) ≅ ⟨D, σ, J⟩` at the level of whole configurations, which is the level ≅ is defined at — and this positive half inherits Proposition 4's hypotheses, journal-closedness included. In the coordination variant it does not, even up to ≅ — and this negative half is independent of (HOST)'s freshness clauses, needing only totality and (NONTRIV), with the premise that (NONTRIV)'s witness state is reachable in the variant (the two calculi share their execution rules and differ only in where a definition lives, so their reachable states coincide): there are operations whose outcomes differ at some state, so ambient bodies `b ≠ b′` differing in such an operation make the same `J` replay to different states under `P[V↦b]` and `P[V↦b′]`, the interpretation held fixed. Even if the positive half were narrowed further, the separation stands, because what it rests on is the existence of a `P` free to vary. ∎ What moves into the record is therefore the *composition*, never the operations — a difference in how much stays ambient, not a categorical "record alone" — and §8 carries the remainder as a limit. On the engine: the durable-execution row of §3.2 — the history holds the exercises and replay re-walks compiled code, so the same history under changed code means something else; and the constitution row, where with the repertoires held fixed, changing nothing but the record is enough. §3.1's (b) is this proposition's question asked of a record — with the engine and the repertoires held fixed, does the record alone determine a replay after which the whole's outcome exists again? — the determination half is proved here, and the outcome half is Proposition 4 together with the measurement of §3.2.

**The four questions, and the one arrow among them.** Over a reachable configuration, three of §2.4's four lines are projections: *what the subject can do* is `dom(D)`; *what counts as its history* is `J`; *what is computed* is `σ`. The fourth — *what the subject does* — is deliberately not in the configuration: performances are the trace's property, and Proposition 3 witnesses their separation at the rules themselves, one premise under two conclusions. Independence is claimed exactly and no further: **no projection is determined by the others except in the one direction Proposition 4 establishes under (HOST)** — `J` determines `D` exactly and `σ` up to fresh names — and that arrow is the paper's finding, not the table's defect: recorded constitution *is* the arrangement in which the history suffices for the rest. What remains free is witnessed pairwise: equal `σ` with different `D` (Observation 1); equal `J` with different executions (Proposition 3's query half); equal `D` with different `J` (runs of different exercise counts). Nothing runs the other way: `σ` does not determine `J` (Observation 1's pair), and `J` does not reveal the query performances at all.
