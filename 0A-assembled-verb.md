...
**Definition (≅, and determination).** Two configurations are **equivalent**, `C ≅ C′`, when some bijection on trace-minted names relates them componentwise, fixing every name not minted during their traces. A pair `(J, ⟦·⟧)` **determines** replay's outcome up to ≅ when every configuration any run of `replay(J)` reaches falls in one ≅-class. Propositions 4 and 5 use exactly these notions.

**Proposition 4 (the record constitutes, up to fresh names — and no closer).** Assume (HOST); let `⟨D, σ, J⟩` be reachable, and let the trace be **journal-closed**: no `act(V, v*)` in `J` carries a name minted during the trace — every journaled argument crossed the plane from outside. Then every run of `replay(J)` reproduces `D` exactly — definitions mint nothing, so that much is a strict equality — and reaches a state `σ_r` with a bijection ρ on minted names such that `ρ(σ_r) = σ`. The bijection is the strongest identity that holds **for every interpretation satisfying (HOST)** — a particular host may happen to repeat its names, and the axioms do not let replay count on it — so §3.1's clause is proved necessary at exactly this strength, not conceded. *Proof sketch.* Induction over the define/command sub-trace with hypothesis `ρ_k(σ_r^k) = σ^k`; query steps change nothing in the configuration, so they drop out. At the next command step both sides execute the same journaled `v*`, which by journal-closedness contains no trace-minted name, so `ρ_k` fixes it; (HOST)'s equivariance applied to `ρ_k` transports the replay's application at `(σ_r^k, v*)` to an application at `(σ^k, v*)` — the live one — whose outcomes therefore correspond under an extension `ρ_{k+1} ⊇ ρ_k` over the step's fresh names, equal in number by (HOST)'s count invariance. The extensions compose because fresh names are new on both sides. ∎ On the engine: the constitution row of §3.2, whose *yes* is stated under exactly this equivalence (§3.1) — and §4.3 measures the axiom's licensed minting live: an identity minted inside a foreign factory differs on every replay while the supplied key holds, which is ρ made visible.

**Where the hypothesis fails, the failure is the finding.** Let a trace-minted name be *captured* — read by the assembler and passed into a later act's journaled arguments. The live application's argument then aliases a name of the live state; the replayed application's argument is the recorded literal, which aliases nothing in the replayed state, since replay minted fresh; and no bijection can both align the states and fix the literal. Replay equivalence breaks — not at the mint, but at the **capture** of the mint. The case is located by the calculus and stated from the engine's own discipline rather than measured as a lab: recorded values are re-injected on replay while minted state returns fresh (§4.3), so a re-injected identity is stale by construction. Durable-execution practice prevents exactly this by design, supplying replay-stable identifiers from the runtime instead (§7.2). And the arrangement's safe channel is the one §5.2's scenario uses: references that flow between acts through the replayed environment are *rebound* on replay rather than recorded, and stay consistent under re-minting. What would violate (HOST) itself is different again — uncaptured I/O, escaped clocks, foreign mutable state — and the paper reports every one of these boundaries rather than absorbing them.
