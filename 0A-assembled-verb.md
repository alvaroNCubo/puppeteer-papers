...
| `WhereReplayEquivalenceBreaksLab` | §4.3, §8, App. C | the calculus's prediction, put to the engine: a minted identity captured into a later act's recorded arguments breaks the composition's invariant on replay (true→**false**), while the same composition referring to it by a supplied key holds (true→true) — the mint fresh in both arms. The order is in the history: the derivation lands in the papers repository at `0001735` (15:08), this lab in the engine worktree at `0813fec` (17:06) the same day |
| `WhatTheRecordCostsLab` | §8 | the record's price, Release, n=10 after a discarded warm-up, median [min..max]: **554** bytes for definition + first exercise, **~112** per additional (exact); replay **4.6 ms** at 100 and **15.0 ms** at 1,000 exercises; **~2,490**/s through the plane vs **~529,000**/s direct — **~210x**, two figures being what repeated invocations support; this is the run published as `data/paper0A-assembled-verb/price-release.log` |
...
**Four idealizations, declared.** *(1)* The define/perform split is primitive; in the engine the two arrive fused in one act and the handler splits them at the reuse seam, and §2.3 measures that it does. *(2)* PERFORM-QRY is a rollback rule: the premise executes the body and the conclusion restores `σ` whole, silently undoing whatever a state-changing operation did. That is a substantive modeling decision, not an observation — the calculus's `σ` is *durable* state, the fold of `J`, so a query body's in-memory mutation has no denotation here. The rule matches the engine measured only because in that engine state *is* reconstruction from `J`; on an engine whose in-memory state outlives acts without entering a record, the rule would be false — and repertoire state held outside `σ` altogether (statics, caches, external stores) rolls back nowhere, on any engine. *(3)* `⟦f⟧` is a relation over one store: no aliasing across repertoires, no shared mutable state outside `σ`, no reentrancy. The shape assumes by construction the tractability of exactly the situation §5.2 exhibits as hard — two repertoires whose only meeting point is the assembler. (HOST) bounds what `⟦f⟧` may *depend on*; this shape bounds what it may *be*. *(4)* `⟦f⟧` is total: no operation fails, so the calculus expresses no failed exercise — misfire and its kin are outside the object.
...
**Proposition 4 (the record constitutes, up to fresh names — and no closer).** Assume (HOST); let `⟨D, σ, J⟩` be reachable, and let the trace be **journal-closed**: no `act(V, v*)` in `J` carries a name minted during the trace — every journaled argument crossed the plane from outside. Then every run of `replay(J)` reproduces `D` exactly — definitions mint nothing, so that much is a strict equality — reproduces `J` (its `def` entries carry no names, and its `act` arguments are journal-closed, so π fixes them: `J_r = J`) — and reaches a state `σ_r` with a bijection π on minted names such that `π(σ_r) = σ`. The three conclusions together are `replay(J) ≅ ⟨D, σ, J⟩`, which is what Proposition 5 cites. The bijection is the strongest identity that holds **for every interpretation satisfying (HOST)** — a particular host may happen to repeat its names, and the axioms do not let replay count on it — so §3.1's clause is proved necessary at exactly this strength, not conceded. *Proof sketch.* Induction over the define/command sub-trace with hypothesis `π_k(σ_r^k) = σ^k`; query steps change nothing in the configuration, so they drop out. At the next command step both sides execute the same journaled `v*`, which by journal-closedness contains no trace-minted name, so `π_k` fixes it; (HOST)'s equivariance applied to `π_k` transports the replay's application at `(σ_r^k, v*)` to an application at `(σ^k, v*)` — the live one — whose outcomes therefore correspond under an extension `π_{k+1} ⊇ π_k` over the step's fresh names, equal in number by (HOST)'s count invariance. The extensions compose because fresh names are new on both sides. ∎ On the engine: the constitution row of §3.2, whose *yes* is stated under exactly this equivalence (§3.1) — and §4.3 measures the axiom's licensed minting live: an identity minted inside a foreign factory differs on every replay while the supplied key holds, which is π made visible.

**Where the hypothesis fails, the failure is the finding — and this one was predicted before it was measured.** Let a trace-minted name be *captured* — read by the assembler and passed into a later act's journaled arguments. The live application's argument then aliases a name of the live state; the replayed application's argument is the recorded literal, which aliases nothing in the replayed state, since replay minted fresh; and no bijection can both align the states and fix the literal. Replay equivalence breaks — not at the mint, but at the **capture** of the mint.

That is a derivation, and the engine was then asked (Appendix A: `WhereReplayEquivalenceBreaksLab.cs`). What was predicted is stated exactly, because the paper's other claims are checkable by a third party and this one would otherwise rest on the author's word. It is not that a supplied key survives replay — `RetrievalIsALookupLab` had measured that two weeks before this calculus existed. It is that a **captured** mint fails, a case no lab had named: the derivation above enters the record in the commit that repairs this proposition's induction, and the lab enters it in a later commit the same day, so the order is a property of the repository rather than a claim in the text (both hashes in Appendix A). Two arms differ in one thing — how a later act refers to what an earlier act minted — and the composition's own invariant is read back live and after replay:

    captured: the minted identity travels as the later act's argument
              invariant live  true      invariant after replay  FALSE
    supplied: a key the assembler chose travels, and the value is found by it
              invariant live  true      invariant after replay  true

What the lab observes is that the composition's own invariant — the register's key is the identity of the payment kept under it — holds live and fails after replay. That the failure *is* a violation of ≅ is the calculus's step, not the lab's reading: a configuration whose invariant fails cannot be related by any renaming to one whose invariant holds, since π maps values to values and cannot make an equality false. The measurement and the inference are kept apart here as everywhere else. The mint is fresh in both arms; only the captured one breaks. This is the paper's one result obtained by reasoning about the arrangement rather than by measuring what it was built to do: the calculus named the failure, and the engine produced it. And the second arm confirms the sufficient condition the calculus offers — if every reference to a minted value travels between acts as a lookup under a key that crossed the plane, rather than as the value itself, the trace is journal-closed and Proposition 4 applies. That is a characterization of when the guarantee holds, not advice about how to write verbs. Durable-execution practice reaches the same safety differently, by supplying replay-stable identifiers from the runtime (§7.2); this arrangement leaves it to what the assembler records.

**What the hypothesis costs the measured arrangements.** For §2.2's flagship it costs nothing: the composition is a single act whose arguments are all supplied, so the trace is journal-closed outright. §5.2's scenario is journaled nowhere — it exercises the two repertoires directly to exhibit what neither can say — so the hypothesis does not reach it. And §5.1's co-binding travels within one body, through the local environment, never through a journaled argument. The hypothesis bites exactly where the lab above puts it: a value minted inside an operation, read out, and recorded as a later act's argument.

What would violate (HOST) itself is different again — uncaptured I/O, escaped clocks, foreign mutable state — and the paper reports every one of these boundaries rather than absorbing them.
...
**What the appendix did to the paper.** It is worth recording that the traffic ran mostly one way. The criterion's replay question was restated in the determination form Proposition 5 proves; Proposition 5 itself forced the relativization the body now carries as a limit, that the record closes over the composition and not the operations; the fresh-value clause of §3.1 stopped being a concession and became a consequence of the host axiom; and the capture boundary, with the lab that measures it, did not exist before the induction of Proposition 4 needed a hypothesis. Four corrections from the small object to the measured arrangement, and none in the other direction — which is the argument for having written it.
