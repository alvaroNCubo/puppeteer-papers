...
**Proposition 4 (the record constitutes, up to fresh names — and no closer).** Assume (HOST), and let `⟨D, σ, J⟩` be reachable. Every run of `replay(J)` reproduces `D` exactly — definitions mint nothing, so that much is a strict equality — and reaches a state `σ_r` for which there is a bijection ρ between the names minted by the original run's define/command steps and the names minted by the replay run, with `ρ(σ_r) = σ`. The bijection is the strongest identity that holds **for every interpretation satisfying (HOST)** — a particular host may happen to repeat its names, and the axioms do not let replay count on it — so §3.1's clause is proved necessary at exactly this strength, not conceded. *Proof sketch.* Induction over the trace from `⟨∅, σ₀, ε⟩`. Query steps change nothing in the configuration, so the configuration after any prefix equals the configuration after its define/command sub-prefix — which is exactly the sequence replay re-walks, with the same journaled `v*`. At each command step, (HOST)'s equivariance and mint-count invariance match the two applications: equal counts on both sides, outcomes agreeing up to a bijection on the fresh names, and the per-step bijections compose because fresh names are disjoint from everything prior. ∎ On the engine: the constitution row of §3.2, whose *yes* is stated under exactly this equivalence (§3.1) — and §4.3 measures the axiom's licensed minting live: an identity minted inside a foreign factory differs on every replay while the supplied key holds, which is ρ made visible. What would violate (HOST) is not minting but the rest of what it closes out — uncaptured I/O, escaped clocks, foreign mutable state — and the paper reports the boundary rather than absorbing it.
...
**Proposition 5 (what the criterion separates is how much stays ambient).** Neither calculus lets the record settle its replay absolutely: re-performing reads `⟦f⟧`, which is code outside `J`, so a changed operation implementation replays the same record to a different state in *either* calculus — the interpretation is a parameter everywhere, which is why §3.1's (b) holds it fixed, along with the given `σ₀`. What separates the calculi is determination beyond `(J, ⟦·⟧)`, stated in the only type replay has — a relation whose outcomes are compared up to renaming of minted names. In the constitution calculus, `(J, ⟦·⟧)` **determines** replay's outcome up to that renaming (Proposition 4). In the coordination variant it does not, even up to renaming: replay there is `replay_P`, and by (NONTRIV) there are operations whose outcomes differ at some state, so ambient bodies `b ≠ b′` differing in such an operation make the same `J` replay to different states under `P[V↦b]` and `P[V↦b′]`, the interpretation held fixed. ∎ What moves into the record is therefore the *composition*, never the operations — a difference in how much stays ambient, not a categorical "record alone" — and §8 carries the remainder as a limit. On the engine: the durable-execution row of §3.2 — the history holds the exercises and replay re-walks compiled code, so the same history under changed code means something else; and the constitution row, where with the repertoires held fixed, changing nothing but the record is enough. §3.1's (b) is this proposition's question asked of a record — with the engine and the repertoires held fixed, does the record alone determine a replay after which the whole's outcome exists again? — the determination half is proved here, and the outcome half is Proposition 4 together with the measurement of §3.2.
