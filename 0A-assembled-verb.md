...
A body invokes operations, never verbs: the calculus has no verb call inside a body, matching the arrangement measured, where a definition's statements name repertoire operations. Composition of verbs out of verbs is neither modeled nor claimed.

**Configurations, and what is given.** `⟨D, σ, J, ν⟩` — definitions `D : Verb ⇀ (x*, b)`; domain state `σ`; the journal `J`, a sequence of entries `def(V, x*, b)` and `act(V, v*)`; and `ν`, a stream of pairwise-distinct names not occurring in `σ₀`. Two constants come with the calculus: an initial state `σ₀` and an initial supply `ν₀`. A configuration is **reachable** when the rules below reach it from `⟨∅, σ₀, ε, ν₀⟩`; every claim quantifies over these.
...
**The host as a parameter — axiom (HOST).** The repertoires enter the calculus as an interpretation governed by one axiom, named once and cited by name:
    ...
    (HOST)  ⟦f⟧ relates (σ, v*) to (σ′, v, N), totally on (σ, v*), where
            N is the finite set of names the application mints:
            N ∩ names(σ, v*) = ∅ (freshness), |N| and the minting positions
            are determined by (σ, v*), and (σ′, v) is determined by (σ, v*)
            up to bijection on N.

Minting is **fresh, not deterministic**: two applications of the same `f` to the same arguments agree except in *which* fresh names they drew. This is the standard move on fresh names, and it is the calculus meeting the paper: §4.3 measures an identity that differs on every replay while the supplied key holds, and (HOST) makes that the licensed behaviour whose guarantee Proposition 4 states. (HOST) is the closure condition §4.3 speaks of, written as an axiom: whatever an execution depends on either crossed the plane (it is in `v*`) or is fresh (it is in `N`). It is a **closure**, and it is worth saying plainly what it closes out: external I/O, randomness beyond the minting the axiom licenses, foreign mutable state reached behind the plane, callbacks, and clock readings that escape it — §4.3's undeclared inbounds. One further axiom gives the calculus something to distinguish, and Proposition 5 cites it:
...
For `x := f(e*)`: the arguments `e*` are read under `ρ` (a variable `ρ` does not bind is stuck), `⟦f⟧` is applied drawing its fresh names from the front of `ν`, and `ρ′ = ρ[x ↦ v]`; for bare `f(e*)`, `ρ′ = ρ`. A body folds its statements left to right, `⟨σ, ρ, ν⟩ ⊢ b ⇓ ⟨σ′, ρ′, ν′⟩ ! v`, emitting the last statement's output.
...
**Rules.**
    ...
    D(V) = (x*, b)    |v*| = |x*|    ⟨σ, [x*↦v*], ν⟩ ⊢ b ⇓ ⟨σ′, ρ′, ν′⟩ ! v
    ─────────────────────────────────────────────────────────────  (PERFORM-CMD)
    ⟨D, σ, J, ν⟩  ─perform V(v*):cmd→  ⟨D, σ′, J·act(V,v*), ν′⟩ ! v

    D(V) = (x*, b)    |v*| = |x*|    ⟨σ, [x*↦v*], ν⟩ ⊢ b ⇓ ⟨σ′, ρ′, ν′⟩ ! v
    ─────────────────────────────────────────────────────────────  (PERFORM-QRY)
    ⟨D, σ, J, ν⟩  ─perform V(v*):qry→  ⟨D, σ, J, ν⟩ ! v

Two readings of PERFORM-QRY are deliberate. The execution happens — the premise is the same judgment as the command's, and the output `v` leaves, which is what keeps a query performance observable at all — and the configuration keeps nothing: not the effect, not the trace, not even the draw on the supply. What a query computes is visible at the boundary as its output and durable nowhere. In the engine measured this is literal, because state *is* reconstruction from `J` (§2.4): what a query changed does not exist after rehydration.

**Three idealizations, declared.** *(1)* The define/perform split is primitive; in the engine the two arrive fused in one act and the handler splits them at the reuse seam, and §2.3 measures that it does. *(2)* PERFORM-QRY is a rollback rule: the premise executes the body and the conclusion restores `σ` and `ν` whole, silently undoing whatever a state-changing operation did. That is a substantive modeling decision, not an observation — the calculus's `σ` is *durable* state, the fold of `J`, so a query body's in-memory mutation has no denotation here. The rule matches the engine measured only because in that engine state *is* reconstruction from `J`; on an engine whose in-memory state outlives acts without entering a record, the rule would be false — and repertoire state held outside `σ` altogether (statics, caches, external stores) rolls back nowhere, on any engine. *(3)* `⟦f⟧` is a relation over one store: no aliasing across repertoires, no shared mutable state outside `σ`, no reentrancy. The shape assumes by construction the tractability of exactly the situation §5.2 exhibits as hard — two repertoires whose only meeting point is the assembler. (HOST) bounds what `⟦f⟧` may *depend on*; this shape bounds what it may *be*.

**Replay.** `replay(J)` is the configuration reached from `⟨∅, σ₀, ε, ν₀⟩` by taking each `def` entry as its DEFINE step and each `act` entry as its PERFORM-CMD step, in `J`'s order. For the coordination variant of Proposition 5, `replay_P(J)` starts instead from `⟨D_P, σ₀, ε, ν₀⟩`, with `D_P` read off the ambient program: the variant's journal holds no `def` entries, so a replay started from `∅` is stuck at its first `act` — which is itself the point, stated operationally: the record alone does not carry what its replay needs.
...
**Proposition 5 (what the criterion separates is how much stays ambient).** Neither calculus makes replay a function of the record absolutely: re-performing reads `⟦f⟧`, which is code outside `J`, so a changed operation implementation replays the same record to a different state in *either* calculus — the interpretation is a parameter everywhere, which is why §3.1's (b) holds it fixed, along with the given `σ₀, ν₀`. What separates the calculi is the remainder beyond `(J, ⟦·⟧)`. In the coordination variant — DEFINE extends the ambient program and appends nothing to `J` — replay is `replay_P`, a function of `(J, ⟦·⟧, P)`: by (NONTRIV) there are operations whose outcomes differ at some state, so two ambient bodies `b ≠ b′` differing in such an operation make the same `J` replay to different states under `P[V↦b]` and `P[V↦b′]`, the interpretation held fixed. In the constitution calculus there is no `P` to vary — the bodies replay executes are read from `def` entries of `J` itself, by inspection of REPLAY. ∎ What moves into the record is therefore the *composition*, never the operations — a difference in how much stays ambient, not a categorical "record alone" — and §8 carries the remainder as a limit. On the engine: the durable-execution row of §3.2 — the history holds the exercises and replay re-walks compiled code, so the same history under changed code means something else; and the constitution row, where with the repertoires held fixed, changing nothing but the record is enough. §3.1's (b) adopts this separation as its input clause — the record alone *modulo the engine and the repertoires, which its wording holds fixed* — so what the criterion asks is what this proposition proves.

**The four questions, and the one arrow among them.** Over a reachable configuration, three of §2.4's four lines are projections: *what the subject can do* is `dom(D)`; *what counts as its history* is `J`; *what is computed* is `σ`. The fourth — *what the subject does* — is deliberately not in the configuration: performances are the trace's property, and Proposition 3 witnesses their separation at the rules themselves, one premise under two conclusions. Independence is claimed exactly and no further: **no projection is determined by the others except in the one direction Proposition 4 establishes** — `J` determines `D` exactly and `σ` up to fresh names — and that arrow is the paper's finding, not the table's defect: recorded constitution *is* the arrangement in which the history suffices for the rest. What remains free is witnessed pairwise: equal `σ` with different `D` (Observation 1); equal `J` with different executions (Proposition 3's query half); equal `D` with different `J` (runs of different exercise counts). Nothing runs the other way: `σ` does not determine `J` (Observation 1's pair), and `J` does not reveal the query performances at all.
