...
This inversion is *structural*, not a measured benefit. The eventual-consistency windows of a verb are, by construction, enumerable from the Reactions declared near it — a checkable property of the source, and the one this section claims. A classical CQRS system locates the same information elsewhere: in the topology of the read store, the synchronisation policy, the lag profile under load, and the application's compensations — knowable, but not from the verb's own text. The contrast is in *where the information lives*, not in a demonstrated improvement: whether per-verb enumerability lowers defect rates or comprehension time is an empirical question this paper does not answer, and it belongs to the deferred case-study (§6.10). The value asserted here is the analytic one — the consistency surface of a verb can be read off its Reactions — not a human-factors result.
