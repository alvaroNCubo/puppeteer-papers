...
1. **Logical, not physical, CQRS.** Within an actor, write and read paths operate on a single in-memory state. `PerformCmd` and `PerformQry` are verbs over the same store; there is no synchronization between separated write and read sides because there are no separated sides. The eventual-consistency contract that defines classical CQRS does not apply at the actor boundary. This is the semantics of a stateful actor (Hewitt, Bishop, & Steiger, 1973; Agha, 1986), not a contribution of this paper; the claim records it as the ground on which §5.3 reframes the consistency vocabulary. *(Verification: code references to `PerformCmd`/`PerformQry` over a shared actor state in §6.)*
...
8. **The continuation contract survives the deployment switch.** Deferred work in flight when a node hands off to its successor is not lost — and the property rests on a mechanism that is in scope, not on the deployment machinery. A Reaction's cursor is durable and replicated with the journal: the per-Reaction checkpoint of §3.6 is persisted in the same store as the journal (`DiaryStorage.GetReactionCheckpoint` returns its *detected* and *confirmed* positions), so it travels wherever the journal replicates. The successor reads the same replicated journal and resumes from the last *confirmed* checkpoint; any deferred work not yet confirmed is re-attempted by the successor rather than dropped — at-least-once across the handoff, exactly as within a single node (§3.6). What is genuinely out of scope here is the deployment-time machinery that makes the handoff *seamless* — the red-black, zero-downtime coordination realized by `Theater.aliveGate`, which governs *when* the successor goes live, not *whether* work survives — treated in a separate companion paper. The cross-actor continuity primitive is likewise treated separately. (The one common operational exception is a coordinated cut-over of a Kafka producer/consumer pair when a message format changes, where both sides must release together.)
...
- **Computation of domain values over the matched events (`Sum`, `Count`, `Average`, arithmetic over payloads) — a deliberate boundary, not a gap.** The matcher's quantifiers shape the *set* of matches — group or de-duplicate it by a captured identifier, bound its size or window (§6.4) — which is correlation, the matcher's own work. They stop deliberately short of computing domain values over what was matched. This is not a shortfall against CEP (§9.3): it is the anti-porosity boundary of §3.3 and [Paper 1](01-anti-porosity.md) applied to the matcher — value computation is *domain* work and lives in the domain library under its legitimate paradigm, not in the Reaction DSL. The Reaction recognizes a trajectory of historical facts (the journaled invocations, [Paper 2](02-program-value-separability.md)) and defers; a decision that needs an aggregate invokes a domain method or decomposes into two Reactions. The cost is real and owned: you cannot fold a running `Sum` into the pattern itself.
