# Builds paper0A-data.zip in the shape Code provenance promises, and the shape the
# earlier papers' archives already have: labs/paper0A-* and data/paper0A-* side by
...
# such zip of the price lab alone came to 246 MB, against 0.01 to 0.72 MB for the archives of
...
# labs/paper0A-labH-recognition/ alone gets that lab's claim of three byte-identical
# journals and none of the evidence for it. That is a real report from a real
# reviewer, not a hypothetical.
#
#   .\paper0A-package.ps1                 -> paper0A-data.zip beside this script
#   .\paper0A-package.ps1 -Out C:\tmp\x.zip
#
...
try {
  ...
  $files = @(git ls-files "labs/paper0A*" "data/paper0A*" "paper0A-assets/*") | Where-Object { $_ -and $_ -notlike '*.zip' }
  if ($files.Count -eq 0) { throw "no tracked paper0A files found. Is this the papers repository?" }
  ...
  $dirty = @(git status --porcelain -- "labs/paper0A*" "data/paper0A*" "paper0A-assets/*") | Where-Object { $_ -and $_ -notlike '*.zip' }
  if ($dirty.Count -gt 0) {
    Write-Warning "$($dirty.Count) uncommitted change(s) under paper0A. The archive will NOT match any commit:"
    $dirty | ForEach-Object { Write-Warning "  $_" }
  }

  $stage = Join-Path ([IO.Path]::GetTempPath()) ("paper0A-pkg-" + [Guid]::NewGuid().ToString('N').Substring(0,8))
  ...
  try {
    ...
    Write-Host ""
    Write-Host ("paper0A-data.zip  {0} files  {1:N2} MB  at commit {2}" -f $files.Count, $size, $head)
