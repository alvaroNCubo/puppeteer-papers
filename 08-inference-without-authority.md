...
*Dependencies. This paper is part of the Puppeteer Papers, a series of self-deposited preprints, and rests on three of them: the actor's speech (Paper 4), the journal as substrate (Paper 5), and the infrastructural-symptom construct it reuses (Paper 6). The argument here is self-contained, but those foundations are established there rather than re-argued.*
...
**Figure 1.** The actor bounded on both sides — a soft, negotiable edge to the domain and a hard, fixed edge to the assembler — with each symptom (§3) marked as a reach across an edge into a decision the reaching party has no standing to make.
